...
export class GroupService {
	...
	async joinGroup(request: GroupIdRequestDto, userId: number): Promise<void> {
		try {
			...
			logger.info(`User ${userId} requested to join group ${request.group_id}.`);
		} catch (error: unknown) {
			dbUtils.handleDatabaseError(error, {
				50001: "The group has already reached its maximum number of members.",
                50002: "User must wait 48 hours before requesting to join the group again.",
	...
	async getPendingJoinRequests(request: GroupIdRequestDto): Promise<PendingJoinRequestResponseDto[]> {
        try {
            const result = await dbUtils.executeStoredProcedure<PendingJoinRequestResponseDto[]>("SP_GetPendingJoinRequests", {
                GroupId: request.group_id,
            });
	...
	async updateJoinRequest(request: UpdateJoinGroupRequestDto): Promise<void> {
        try {
            ...
            logger.info(`Join request ${request.request_id} updated to status ${request.ref_join_request_status_id}.`);
        } catch (error: unknown) {
            dbUtils.handleDatabaseError(error, {
	...
	async transferOwnership(request: TransferOwnershipRequestDto, ownerId: number): Promise<void> {
        try {
            ...
            logger.info(`Ownership of group ${request.group_id} transferred from user ${ownerId} to user ${request.new_owner_id}.`);
        } catch (error: unknown) {
            dbUtils.handleDatabaseError(error, {
